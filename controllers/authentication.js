    ...
    crypto.randomBytes(48, (err, buffer) => {
      ...
      existingUser.save((err) => {
        ...
        const message = {
          ...
          nodemailer.sendEmail(existingUser.email, message);

        return res.status(200).json({ message: 'Please check your email for the link to reset your password.', resetToken: resetToken });
      });
    });
  });
};
  ...
  User.findOne({ resetPasswordToken: req.params.token, resetPasswordExpires: { $gt: Date.now() } }, (err, resetUser) => {
    ...
    resetUser.save((err) => {
      ...
      const message = {
        ...
        nodemailer.sendEmail(resetUser.email, message);
